...
import {
  isHomeFilterApplied,
  saveHomeScreenData,
} from '../../Redux/Action/homScreenActions';

const EventFilter = ({navigation, route}: any) => {
  ...
  const handleApplyFilter = () => {
    const musicTypeIds = selectedMusicTypes.map(music => music._id).join(',');
    const eventTypeIds = selectedEventType.map(music => music._id).join(',');
    const venueTypeIds = selectedVenueType.map(music => music._id).join(',');
    const languages = selectedLang.join(',');

    // Validation checks
    if (selectedValues2[1] === 0) {
      showError('Please specify the maximum distance.');
      return;
    }
    if (!venueTypeIds) {
    ...
    if (!musicTypeIds) {
      showError('Please select at least one music type.');
      return;
    }
    if (!languages) {
    ...
    const filterData = {
      music_type: musicTypeIds,
      venue_id: venueTypeIds,
      event_id: eventTypeIds,
      language: languages,
      max_distance: selectedValues2[1],
      userLatitude: route.params.lat,
      userLongitude: route.params.lon,
      lowest_price: selectedValue[0].toString(),
      highest_price: selectedValue[1].toString(),
    };

    getHomedataWithFilters(filterData)
      .then((res: any) => {
        if (res.status === 200) {
          isHomeFilterApplied(true);
  ...
  const resetButton = () => {
    ...
    setSelected3([0, 0]);
    setSelectedValues2([0, 0]);
    getHomedata(route.params.lat, route.params.lon, 'all')
      .then((res: any) => {
        saveHomeScreenData(res.events);
        navigation.goBack();
      })
                    ...
                    <View
                      ...
                      <View
                        ...
                        <DropComponentNew
                          ...
                          selectedItems={selectedEventType.map(
                            item => item._id,
                          )}
                        />
                      </View>
                    ...
                    <View
                      ...
                      <View
                        ...
                        <DropComponentNew
                          ...
                          selectedItems={selectedVenueType.map(
                            item => item._id,
                          )}
                        />
                      </View>
                    ...
                    <View
                      ...
                      <View
                        ...
                        <DropComponentNew
                          ...
                          selectedItems={selectedMusicTypes.map(
                            item => item._id,
                          )}
                        />
                      </View>
                            ...
                            style={[
                              styles.flatcontainer,
                              {
                                backgroundColor: selectedItemsSign?.includes(
                                  ...
                                  : Colors.tranparent,
                                borderWidth: 1,
                                borderColor: selectedItemsSign?.includes(
                                  item?.id,
                                )
                                  ? Colors.tranparent
                                  : Colors.darkPink,
