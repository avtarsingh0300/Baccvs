...
const SetPriceScreen = ({navigation}: any) => {
  const renderPrice = ({item, index}: any) => (
    <View style={{marginTop: index > 0 ? 20 : 0}}>
      ...
      <SizeBox size={5} />
      <Text style={[styles.eurtxt, {color: Colors.white, width: '90%'}]}>
        For an hour, DJs in your category typically charge between 100 and 250
        euros.
      </Text>
      <SizeBox size={10} />
      <Text style={{...commonStyles.font16Regular}}>
        Profit after service fees
      </Text>
      <SizeBox size={10} />
      <TouchableOpacity activeOpacity={0.7} style={styles.pricebtn}>
        <View></View>
